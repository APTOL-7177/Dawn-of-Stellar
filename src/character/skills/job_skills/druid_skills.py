...
def create_druid_skills():
    ...
    healing_forest.effects = [
        HealEffect(HealType.HP, percentage=0.57, is_party_wide=True),  # 치유의 숲 (파티 힐)
...
def register_druid_skills(skill_manager):
    ...
    teamwork.effects = [
        # 아군 전체 최대 HP의 50% 회복
        HealEffect(percentage=0.5, is_party_wide=True),
        # 자신 자연 포인트 +50
        GimmickEffect(GimmickOperation.ADD, "nature_points", 50),
        # 다음 변신 지속시간 2배 (메타데이터로 처리)
    ]
