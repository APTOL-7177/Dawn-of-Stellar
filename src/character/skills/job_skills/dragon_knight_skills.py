"""Dragon Knight Skills - 용기사 스킬 (화염/용의 힘 시스템)"""
from src.character.skills.skill import Skill
from src.character.skills.teamwork_skill import TeamworkSkill
from src.character.skills.effects.damage_effect import DamageEffect, DamageType
from src.character.skills.effects.gimmick_effect import GimmickEffect, GimmickOperation
from src.character.skills.effects.buff_effect import BuffEffect, BuffType
from src.character.skills.effects.taunt_effect import TauntEffect
from src.character.skills.effects.status_effect import StatusEffect, StatusType  # 화상 효과용 추가
from src.character.skills.costs.mp_cost import MPCost
from src.character.skills.costs.stack_cost import StackCost

def create_dragon_knight_skills():
    """용기사 10개 스킬 생성 (화염/용의 힘 시스템)"""

    skills = []

    # 1. 기본 BRV: 화염 베기
    flame_slash = Skill("dragon_knight_flame_slash", "화염 베기", "[화염 속성] 용의 힘 스택 획득 + 화상 부여")
    flame_slash.effects = [
        DamageEffect(DamageType.BRV, 1.4, stat_type="physical", element="fire"),
        GimmickEffect(GimmickOperation.ADD, "dragon_power", 1, max_value=5),
        StatusEffect(StatusType.BURN, duration=3, value=0.1, damage_stat="magic", damage_multiplier=0.15, chance=0.5)  # 50% 화상
    ]
    flame_slash.costs = []
    flame_slash.sfx = ("skill", "fire")
    flame_slash.metadata = {"element": "fire", "dragon_power_gain": 1, "inflicts_burn": True}
    skills.append(flame_slash)

    # 2. 기본 HP: 용의 돌진
    dragon_dive = Skill("dragon_knight_dragon_dive", "용의 돌진", "용의 힘 소비 공격 + 화상 부여")
    dragon_dive.effects = [
        DamageEffect(DamageType.HP, 1.25, stat_type="physical", element="fire"),
        GimmickEffect(GimmickOperation.CONSUME, "dragon_power", 1),
        StatusEffect(StatusType.BURN, duration=3, value=0.15, damage_stat="magic", damage_multiplier=0.2, chance=0.6)  # 60% 화상
    ]
    dragon_dive.costs = []  # 기본 공격은 MP 소모 없음
    dragon_dive.sfx = ("combat", "damage_high")  # 용의 돌진
    dragon_dive.metadata = {"dragon_power_cost": 1, "inflicts_burn": True}
    skills.append(dragon_dive)

    # 3. 화염 숨결
    fire_breath = Skill("dragon_knight_fire_breath", "화염 숨결", "[화염 속성] 광역 화염 피해 + 화염 내성 감소")
    fire_breath.effects = [
        DamageEffect(DamageType.BRV, 1.35, stat_type="physical", element="fire"),
        GimmickEffect(GimmickOperation.ADD, "dragon_power", 1, max_value=5),
        StatusEffect("curse", duration=3, value=0.2, chance=1.0, metadata={"stat_mod": {"fire_resistance": -0.2}}),  # 화염 내성 20% 감소
        StatusEffect(StatusType.BURN, duration=3, value=0.15, damage_stat="magic", damage_multiplier=0.2, chance=0.7)  # 70% 화상
    ]
    fire_breath.costs = []
    fire_breath.target_type = "all_enemies"
    fire_breath.is_aoe = True
    fire_breath.sfx = ("skill", "fire3")
    fire_breath.metadata = {"element": "fire", "dragon_power_gain": 1, "aoe": True, "inflicts_burn": True}
    skills.append(fire_breath)

    # 4. 작열 강타
    burning_strike = Skill("dragon_knight_burning_strike", "작열 강타", "[화염 속성] 용의 힘 2스택 소비 + 100% 화상")
    burning_strike.effects = [
        DamageEffect(DamageType.BRV, 2.0, stat_type="physical", element="fire"),
        DamageEffect(DamageType.HP, 1.5, stat_type="physical", element="fire"),
        GimmickEffect(GimmickOperation.CONSUME, "dragon_power", 2),
        StatusEffect(StatusType.BURN, duration=4, value=0.2, damage_stat="magic", damage_multiplier=0.25, chance=1.0)  # 100% 화상
    ]
    burning_strike.costs = [MPCost(5), StackCost("dragon_power", 2)]
    burning_strike.sfx = ("combat", "critical")
    burning_strike.metadata = {"element": "fire", "dragon_power_cost": 2, "inflicts_burn": True}
    skills.append(burning_strike)

    # 5. 용의 분노
    dragon_rage = Skill("dragon_knight_dragon_rage", "용의 분노", "용표 1개 획득 + 버프")
    dragon_rage.effects = [
        GimmickEffect(GimmickOperation.ADD, "dragon_marks", 1, max_value=3),
        BuffEffect(BuffType.ATTACK_UP, 0.4, duration=4, target="self")
    ]
    dragon_rage.costs = [MPCost(6)]
    dragon_rage.target_type = "self"
    dragon_rage.sfx = ("character", "status_buff")
    dragon_rage.metadata = {"dragon_mark_gain": 1, "buff": True}
    skills.append(dragon_rage)

    # 6. 용의 비늘
    dragon_scales = Skill("dragon_knight_dragon_scales", "용의 비늘", "방어 태세")
    dragon_scales.effects = [
        BuffEffect(BuffType.DEFENSE_UP, 0.5, duration=3, target="self"),
        BuffEffect(BuffType.MAGIC_DEFENSE_UP, 0.5, duration=3, target="self"),
        GimmickEffect(GimmickOperation.ADD, "dragon_power", 2, max_value=5)
    ]
    dragon_scales.costs = [MPCost(3)]
    dragon_scales.target_type = "self"
    dragon_scales.sfx = ("skill", "protect")
    dragon_scales.metadata = {"buff": True, "dragon_power_gain": 2}
    skills.append(dragon_scales)

    # 7. 지옥불 폭발
    inferno_burst = Skill("dragon_knight_inferno_burst", "지옥불 폭발", "[화염 속성] 용의 힘 3스택 소비 광역 폭발 + 100% 화상")
    inferno_burst.effects = [
        DamageEffect(DamageType.BRV_HP, 2.1, stat_type="physical", element="fire"),
        GimmickEffect(GimmickOperation.CONSUME, "dragon_power", 3),
        StatusEffect(StatusType.BURN, duration=4, value=0.25, damage_stat="magic", damage_multiplier=0.3, chance=1.0)  # 100% 화상
    ]
    inferno_burst.costs = [MPCost(7), StackCost("dragon_power", 3)]
    inferno_burst.target_type = "all_enemies"
    inferno_burst.is_aoe = True
    inferno_burst.sfx = ("skill", "flare")
    inferno_burst.metadata = {"element": "fire", "dragon_power_cost": 3, "aoe": True, "inflicts_burn": True}
    skills.append(inferno_burst)

    # 8. 용의 날개
    dragon_wings = Skill("dragon_knight_dragon_wings", "용의 날개", "용의 힘 4스택 소비 대공격 + 화상")
    dragon_wings.effects = [
        DamageEffect(DamageType.BRV, 2.8, stat_type="physical", element="fire"),
        DamageEffect(DamageType.HP, 2.0, stat_type="physical", element="fire"),
        GimmickEffect(GimmickOperation.CONSUME, "dragon_power", 4),
        StatusEffect(StatusType.BURN, duration=3, value=0.2, damage_stat="magic", damage_multiplier=0.25, chance=0.8)  # 80% 화상
    ]
    dragon_wings.costs = [MPCost(9), StackCost("dragon_power", 4)]
    dragon_wings.sfx = ("combat", "damage_high")
    dragon_wings.metadata = {"dragon_power_cost": 4, "inflicts_burn": True}
    skills.append(dragon_wings)

    # 9. 용의 포효
    dragon_roar = Skill("dragon_knight_dragon_roar", "용의 포효", "용의 힘 회복 + 파티 차원의 외침")
    dragon_roar.effects = [
        GimmickEffect(GimmickOperation.ADD, "dragon_power", 3, max_value=5),
        BuffEffect(BuffType.ATTACK_UP, 0.4, duration=4, is_party_wide=True),
        BuffEffect(BuffType.DEFENSE_UP, 0.3, duration=4, is_party_wide=True),
        BuffEffect(BuffType.MAGIC_DEFENSE_UP, 0.3, duration=4, is_party_wide=True)
    ]
    dragon_roar.costs = [MPCost(7)]
    dragon_roar.target_type = "party"
    dragon_roar.sfx = ("skill", "roar")
    dragon_roar.metadata = {"dragon_power_gain": 3, "buff": True, "party_wide": True}
    skills.append(dragon_roar)

    # 10. 용의 각인 (용표 1개 소비 - 강력한 단일 공격)
    dragon_mark_strike = Skill("dragon_knight_dragon_mark_strike", "용의 각인", "용표 1개 소비하여 강력한 일격 + 100% 화상")
    dragon_mark_strike.effects = [
        DamageEffect(DamageType.BRV, 3.0, stat_type="physical", element="fire"),
        DamageEffect(DamageType.HP, 3.5, stat_type="physical", element="fire"),
        GimmickEffect(GimmickOperation.CONSUME, "dragon_marks", 1),
        StatusEffect(StatusType.BURN, duration=4, value=0.25, damage_stat="magic", damage_multiplier=0.3, chance=1.0)  # 100% 화상
    ]
    dragon_mark_strike.costs = [MPCost(8), StackCost("dragon_marks", 1)]
    dragon_mark_strike.sfx = ("combat", "critical")
    dragon_mark_strike.metadata = {"dragon_mark_cost": 1, "high_damage": True, "inflicts_burn": True}
    skills.append(dragon_mark_strike)

    # 11. 용의 폭풍 (용표 2개 소비 - 광역 공격)
    dragon_storm = Skill("dragon_knight_dragon_storm", "용의 폭풍", "[화염 속성] 용표 2개 소비하여 광역 화염 폭풍 + 100% 화상")
    dragon_storm.effects = [
        DamageEffect(DamageType.BRV, 2.4, stat_type="physical", element="fire"),
        DamageEffect(DamageType.HP, 3.0, stat_type="physical", element="fire"),
        GimmickEffect(GimmickOperation.CONSUME, "dragon_marks", 2),
        StatusEffect(StatusType.BURN, duration=4, value=0.3, damage_stat="magic", damage_multiplier=0.35, chance=1.0)  # 100% 화상
    ]
    dragon_storm.costs = [MPCost(12), StackCost("dragon_marks", 2)]
    dragon_storm.target_type = "all_enemies"
    dragon_storm.is_aoe = True
    dragon_storm.sfx = ("skill", "flare")
    dragon_storm.metadata = {"element": "fire", "dragon_mark_cost": 2, "aoe": True, "inflicts_burn": True}
    skills.append(dragon_storm)

    # 12. 궁극기: 진 드래곤 (용표 3개 소비 - 드래곤 변신)
    ultimate = Skill("dragon_knight_ultimate", "진 드래곤", "[화염 속성] 용표 3개로 용으로 변신하여 파멸의 불길 + 100% 화상")
    ultimate.effects = [
        DamageEffect(DamageType.BRV, 3.0, stat_type="physical", element="fire"),
        DamageEffect(DamageType.BRV, 2.0, stat_type="physical", element="fire"),
        DamageEffect(DamageType.HP, 4.5, stat_type="physical", element="fire"),
        BuffEffect(BuffType.ATTACK_UP, 0.8, duration=6, target="self"),
        BuffEffect(BuffType.CRITICAL_UP, 0.6, duration=6, target="self"),
        BuffEffect(BuffType.DEFENSE_UP, 0.4, duration=6, target="self"),
        BuffEffect(BuffType.MAGIC_DEFENSE_UP, 0.4, duration=6, target="self"),
        GimmickEffect(GimmickOperation.ADD, "dragon_marks", 1, max_value=3),
        GimmickEffect(GimmickOperation.CONSUME, "dragon_marks", 3),
        StatusEffect(StatusType.BURN, duration=5, value=0.35, damage_stat="magic", damage_multiplier=0.4, chance=1.0)  # 100% 강력한 화상
    ]
    ultimate.costs = [MPCost(25), StackCost("dragon_marks", 3)]
    ultimate.is_ultimate = True
    ultimate.cooldown = 20
    ultimate.target_type = "all_enemies"
    ultimate.is_aoe = True
    ultimate.sfx = ("skill", "limit_break")
    ultimate.metadata = {"ultimate": True, "element": "fire", "dragon_transform": True, "dragon_mark_cost": 3, "dragon_mark_gain": 1, "aoe": True, "buff": True, "inflicts_burn": True}
    skills.append(ultimate)

    # 팀워크 스킬: 드래곤의 포효
    teamwork = TeamworkSkill(
        "dragon_knight_teamwork",
        "드래곤의 포효",
        "[화염 속성] 전체 적 도발 (2턴, 저항 불가) + 자신 방어력/마방 1.4배 (3턴) + 반격 활성화",
        gauge_cost=225)
    teamwork.effects = [
        BuffEffect(BuffType.ATTACK_UP, 0.6, duration=3, is_party_wide=True),
        DamageEffect(DamageType.HP, 1.5, stat_type="physical", element="fire"),
        BuffEffect(BuffType.DEFENSE_UP, 0.3, duration=3, is_party_wide=True),
        BuffEffect(BuffType.MAGIC_DEFENSE_UP, 0.3, duration=3, is_party_wide=True)
    ]
    teamwork.target_type = "all_enemies"
    teamwork.is_aoe = True
    teamwork.costs = [MPCost(0)]
    teamwork.sfx = ("skill", "teamwork")
    teamwork.metadata = {"teamwork": True, "element": "fire", "chain": True, "buff": True, "dragon": True}
    skills.append(teamwork)

    return skills

def register_dragon_knight_skills(skill_manager):
    """용기사 스킬 등록"""
    skills = create_dragon_knight_skills()
    for skill in skills:
        skill_manager.register_skill(skill)

    # 팀워크 스킬: 드래곤의 포효
    return [s.skill_id for s in skills]