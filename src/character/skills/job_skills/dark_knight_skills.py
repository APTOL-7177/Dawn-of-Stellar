...
def create_dark_knight_skills():
    ...
    # 9. 어둠의 재생
    dark_regeneration = Skill("dark_knight_dark_regeneration", "어둠의 재생", "충전 20 소모하여 HP/BRV 회복")
    ...
    teamwork.effects = [
        GimmickEffect(GimmickOperation.ADD, "absorbed_hp", 25, max_value=100),  # HP 25% 흡수 표시
        BuffEffect(BuffType.ATTACK_UP, 0.8, duration=1, target="self"),  # 다음 공격 데미지 1.8배
    ]
    teamwork.target_type = "party"
    teamwork.is_aoe = True
    teamwork.costs = [MPCost(0)]
    teamwork.sfx = ("skill", "teamwork")
    teamwork.metadata = {"teamwork": True, "chain": True, "dark_knight": True}
