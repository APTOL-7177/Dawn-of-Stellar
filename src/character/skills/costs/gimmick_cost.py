...
class GimmickCost(SkillCost):
    ...
    def __init__(self, field: str, amount: int, min_check: bool = False):
        """
        Args:
            field: 기믹 필드명 (예: "combo", "ammo", "rage")
            amount: 소모량 (min_check=True인 경우 최소 요구량)
            min_check: True인 경우 소모하지 않고 최소치만 확인
        """
