...
def serialize_dungeon(dungeon: Any, enemies: List[Any] = None) -> Dict[str, Any]:
    ...
    # 환경 효과 직렬화 (멀티플레이 동기화용)
    env_effects_data = []
    if hasattr(dungeon, 'environment_effect_manager') and dungeon.environment_effect_manager:
        for effect_type, effect in dungeon.environment_effect_manager.active_effects.items():
            effect_dict = {
                "effect_type": effect.effect_type.value,
                "intensity": effect.intensity,
                "affected_tiles": [list(t) for t in effect.affected_tiles] if effect.affected_tiles else [],
                "metadata": effect.metadata or {}
            }
            env_effects_data.append(effect_dict)
...
def deserialize_dungeon(dungeon_data: Dict[str, Any]) -> Tuple[Any, List[Any]]:
    ...
    # 환경 효과 복원 (멀티플레이 동기화용)
    env_effects_restored = 0
    for env_data in dungeon_data.get("environmental_effects", []):
        try:
            from src.world.environmental_effects import (
                EnvironmentalEffect, EnvironmentalEffectType, EnvironmentalEffectManager
            )
            effect_type = EnvironmentalEffectType(env_data["effect_type"])
            effect = EnvironmentalEffect(
                effect_type=effect_type,
                intensity=env_data.get("intensity", 1.0),
                metadata=env_data.get("metadata", {})
            )
            # affected_tiles 복원 ([x, y] 리스트 → (x, y) 튜플 집합)
            tiles = env_data.get("affected_tiles", [])
            effect.affected_tiles = {(t[0], t[1]) for t in tiles} if tiles else set()

            # 던전의 환경 효과 매니저에 추가
            if not hasattr(dungeon, 'environment_effect_manager') or not dungeon.environment_effect_manager:
                dungeon.environment_effect_manager = EnvironmentalEffectManager()
            dungeon.environment_effect_manager.add_effect(effect)
            env_effects_restored += 1
        except Exception as e:
            logger.warning(f"환경 효과 복원 실패: {e}")

    logger.info(f"던전 복원 완료: {len(harvestables)}개의 채집 오브젝트, {len(enemies)}마리의 적, {env_effects_restored}개의 환경 효과 복원됨")
