...
JOB_STRATEGIES: Dict[str, JobStrategy] = {
    ...
    "elementalist": JobStrategy(
        ...
        job_name="정령술사",
        role="dps_magical",
        priority_actions=["skill_spirit_summon", "skill_spirit_burst", "skill_fusion_firestorm", "skill_spirit_release"],
        gimmick_rules=[
            "spirit_summon 변형 선택으로 4정령 소환: fire/water/wind/earth",
            "융합 requires 충족 우선 — 활성 정령이 융합 한쪽이면 빠진 쪽을 소환",
            "활성 정령이 fire+wind면 fusion_firestorm을 먼저 사용",
            "spirit_burst로 정령 폭발 — 단일 정령 폭딜",
            "수동 UI는 좌/우로 상황별 4정령 직접 선택",
        ],
        target_bias="highest_brv",
        emergency_threshold=0.3,
    ),
    ...
    "engineer": JobStrategy(
        ...
        job_name="엔지니어",
        role="utility",
        priority_actions=["skill_turret_shot", "skill_deploy_turret", "skill_cooling_vent", "skill_mega_blaster"],
        gimmick_rules=[
            "공격 스킬 사용마다 열(Heat) 게이지 증가, 포탑 1기당 열 +1/턴",
            "deploy_turret 좌/우 변형 선택: normal(무료)/fire(MP6)/ice(MP7)/explosive(MP10)/heal(MP12)",
            "열 50~79 유지권 — 80+ 도달 전 cooling_vent(열 -30) 사용",
            "mega_blaster는 turret_count 합산 스케일링 — 포탑 설치 후 사용",
        ],
        target_bias="highest_brv",
        emergency_threshold=0.3,
    ),
