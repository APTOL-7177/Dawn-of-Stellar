...
class MultiplayerPartySetup:
    ...
    def _handle_job_selection_complete(self, message: Any, sender_id: Optional[str] = None):
        ...
        try:
            ...
            # 파티 데이터 수신 및 저장 (호스트: 전투 참여자 수집용)
            party_data = message.data.get("party_data", []) if message.data else []
            if party_data and player_id in self.session.players:
                mp_player = self.session.players[player_id]
                received_party = []
                for member_data in party_data:
                    member = PartyMember(
                        job_id=member_data.get("job_id", ""),
                        job_name=member_data.get("job_name", ""),
                        character_name=member_data.get("character_name", ""),
                        stats=member_data.get("stats", {}),
                        selected_traits=member_data.get("selected_traits", []),
                        player_id=player_id
                    )
                    received_party.append(member)
                mp_player.party = received_party
                self.logger.info(
                    f"플레이어 {player_id}의 파티 데이터 저장: {len(received_party)}명 "
                    f"({', '.join(m.job_name for m in received_party)})"
                )
    ...
    def _send_job_selection_complete(self):
        ...
        try:
            ...
            # 파티 데이터 직렬화 (호스트에게 전송 - 전투 참여자 수집용)
            party_data = []
            for member in self.party:
                party_data.append({
                    ...
                    "stats": member.stats if hasattr(member, 'stats') else {},
                    "selected_traits": member.selected_traits if hasattr(member, 'selected_traits') else [],
                    "player_id": self.local_player_id
                })
            
            message = MessageBuilder.job_selection_complete(self.local_player_id, party_data=party_data)
            # 비동기 브로드캐스트
            import asyncio
            if self.is_host:
                # 호스트 자신도 완료 목록에 추가
