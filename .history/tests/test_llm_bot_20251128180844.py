...
def test_llm_bot_decision():
    ...
    if not test_ollama_connection_sync():
        ...
        # 폴백 테스트
        bot = create_llm_bot("테스트봇", "warrior", "qwen3:1.7b")
        state = create_mock_combat_state()
    ...
    # LLM 봇 생성 (thinking 모드 비활성화로 빠른 응답)
    bot = create_llm_bot("전사봇", "warrior", "qwen3:1.7b", enable_thinking=False)
...
def test_multiple_scenarios():
    ...
    bot = create_llm_bot("테스트봇", "warrior", "qwen3:1.7b", enable_thinking=False)
...
def test_play_styles():
    """플레이 스타일 테스트"""
    print("\n" + "=" * 60)
    print("🎭 플레이 스타일 테스트")
    print("=" * 60)
    
    styles = [PlayStyle.AGGRESSIVE, PlayStyle.DEFENSIVE, PlayStyle.SPEEDRUN]
    ...
    for style in styles:
        ...
        print(f"\n[스타일: {style.value}]")
        print(f"  생성 완료: {bot.bot_name}")
        print(f"  설정: {bot.config.play_style.value}")
        bot.shutdown()
...
def test_battle_memory():
    ...
    bot = create_llm_bot("기억봇", "warrior", "qwen3:1.7b")
    
    # 적 정보 기억
    bot.add_enemy_weakness("드래곤", "빙결 속성")
    bot.add_enemy_pattern("드래곤", "HP 50% 이하에서 화염 브레스 사용")
    bot.remember_battle("드래곤", won=True, notes=["탱커가 도발하면서 힐러가 지속 힐"])
    bot.remember_battle("드래곤", won=False, notes=["화염 브레스에 전멸"])
    
    print(f"\n[드래곤 전투 기억]")
    memory = bot.battle_memories.get("드래곤")
    if memory:
        print(f"  전적: {memory.times_won}/{memory.times_fought}")
        print(f"  약점: {memory.weaknesses}")
        print(f"  패턴: {memory.attack_patterns}")
        print(f"  성공 전략: {memory.successful_strategies}")
        print(f"  실패 전략: {memory.failed_strategies}")
    
    # 컨텍스트 생성 테스트
    context = bot.get_memory_context(["드래곤"])
    print(f"\n[컨텍스트 생성 결과]")
    print(context)
...
def test_party_bots():
    ...
    party_config = [
        {"name": "탱커", "job": "knight"},
        {"name": "힐러", "job": "cleric"},
        {"name": "마법사", "job": "archmage"},
        {"name": "암살자", "job": "assassin"},
    ]
    
    bots = create_party_bots(party_config, style=PlayStyle.BALANCED)
    
    print(f"\n[생성된 파티 ({len(bots)}명)]")
    for bot in bots:
        stats = bot.get_stats()
        print(f"  - {stats['bot_name']} ({stats['job']}): {stats['play_style']}")
...
def test_available_jobs():
    ...
    jobs = get_available_jobs()  # default 제외
    print(f"\n총 {len(jobs)}개 직업:")
    ...
    # 34개 직업 확인
    assert len(jobs) == 34, f"직업 수가 34개가 아닙니다: {len(jobs)}"
...
def main():
    """메인 테스트"""
    print("\n" + "=" * 60)
    print("🎮 LLM 플레이어 봇 테스트 시작 (업그레이드 버전)")
    print("=" * 60)
    
    tests = [
        ("프롬프트 생성", test_prompt_generation),
        ("Ollama 연결", test_ollama_connection),
        ("플레이 스타일", test_play_styles),
        ("전투 기억", test_battle_memory),
        ("파티 봇 생성", test_party_bots),
        ("직업 목록", test_available_jobs),
