...
base_damage = max(1, stat_modifier × skill_multiplier × brv_damage_multiplier × element_bonus)
level_multiplier = 1.0 + (attacker_level × 0.3)
base_damage = base_damage × level_multiplier
variance = random(0.9 ~ 1.1)
final_damage = base_damage × variance × critical_bonus(1.5x)
```
- **방어 관통(pierce)**: defender_def를 % 또는 고정수치만큼 사전 감소
- **원소 보너스**: 속성 상성에 따른 배율 적용
- `src/combat/damage_calculator.py`에서 상세 구현.

## 전투 디버깅 포인트
1. **데미지 0**: 방어력 > 공격력, multiplier 누락, stat_base 잘못 지정
