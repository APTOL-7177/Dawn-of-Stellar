...
def run_ai_spectate_mode(console: tcod.console.Console, context: tcod.context.Context) -> Dict[str, Any]:
    ...
    try:
        ...
        add_ai_commentary(f"🧪 LLM 테스트: {test_action.reasoning}")
        logger.info(f"[AI] LLM 테스트 성공: {test_action.action_type}")
    ...
    def ai_exploration_input(exploration_sys, party, inv) -> GameAction:
        ...
        try:
            ...
            if not current_path:
                ...
                # LLM 이동 결정 - 디버깅 로그 추가
                try:
                    log_action(f"LLM 호출 시작: 위치({px}, {py})")
                    ...
                    log_action(f"LLM 결과: direction={action.direction}, target={action.target_position}")
                except Exception as e:
    ...
    def ai_combat_input(ui, combat_manager, current_char, inv) -> GameAction:
        ...
        try:
            ...
            try:
                ...
                log_action(f"LLM 전투: {current_char.name} -> {action_name}")
