...
def run_ai_spectate_mode(console: tcod.console.Console, context: tcod.context.Context) -> Dict[str, Any]:
    ...
    try:
        from src.town.facilities.shop import give_starting_equipment
        give_starting_equipment(character_party, inventory)
    ...
    def ai_exploration_input(exploration_sys, party, inv) -> GameAction:
        ...
        try:
            ...
            if is_town:
                # 마을에서 현재 타일에 건물이 있으면 상호작용
                tile = dungeon.get_tile(px, py)
                if tile and hasattr(tile, 'building') and tile.building:
                    building = tile.building
                    log_action(f"마을: {building.name} 방문")
                    add_ai_commentary(f"🏠 {building.name} 방문!")
                    # 상호작용 (Z키)
                ...
                if party_hp < 70:
                    for b in town_map.buildings:
                        if b.building_type.value == "inn":
                            # 여관 방향으로 이동
                            if b.x != px or b.y != py:
                                dx = 1 if b.x > px else (-1 if b.x < px else 0)
                                dy = 1 if b.y > py else (-1 if b.y < py else 0)
                                if dy < 0:
