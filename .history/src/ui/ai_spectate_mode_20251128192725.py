...
def run_ai_spectate_mode(console: tcod.console.Console, context: tcod.context.Context) -> Dict[str, Any]:
    ...
    def auto_equip_best_gear(party, inv):
        """최적 장비 자동 장착 (더 좋은 장비로 교체, 내구도 고려)"""
        equipped_count = 0
    ...
    # 초기 장비 장착
    try:
        equipped_count = auto_equip_best_gear(character_party, inventory)
        if equipped_count > 0:
    ...
    while running:
        ...
        if result == "quit":
            ...
            if combat_result == CombatState.VICTORY:
                ...
                # 전투 후 장비 업그레이드 체크
                try:
                    upgrade_count = auto_equip_best_gear(character_party, inventory)
                    if upgrade_count > 0:
