...
def run_ai_spectate_mode(console: tcod.console.Console, context: tcod.context.Context) -> Dict[str, Any]:
    ...
    # 마을 UI 실행 (AI가 자동 행동)
    from src.ui.town_ui import run_town
    
    def ai_town_input(town_ui, party, inv, town_mgr):
        """AI 마을 행동 결정"""
        import random
        import time
        ...
        if not alive:
            return GameAction.CONFIRM  # 던전 출발
        
        party_hp = sum(c.current_hp / c.max_hp * 100 for c in alive) / len(alive)
        
        # HP가 낮으면 휴식
        if party_hp < 80 and inv.gold >= 50:
            add_ai_commentary("💤 휴식 선택")
            time.sleep(0.5)
            return GameAction.MENU  # 휴식 메뉴
        
        # 기본: 던전 출발
        add_ai_commentary("⚔️ 던전 출발!")
        time.sleep(0.5)
        return GameAction.CONFIRM
    
    # 마을 스킵하고 바로 던전 진입 (마을 UI는 복잡하므로 일단 스킵)
    ...
    floor_number = game_stats.get("next_dungeon_floor", 1)
