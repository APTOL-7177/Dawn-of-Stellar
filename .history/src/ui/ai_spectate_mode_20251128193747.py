...
def run_ai_spectate_mode(console: tcod.console.Console, context: tcod.context.Context) -> Dict[str, Any]:
    ...
    def ai_exploration_input(exploration_sys, party, inv) -> GameAction:
        ...
        try:
            ...
            # 70% 확률로 탐험 (미탐험 지역), 30% 확률로 계단 방향
            explore_mode = rnd.random() < 0.7
            ...
            if explore_mode:
                ...
                for game_action, dx, dy in directions:
                    ...
                    if tile and tile.walkable:
                        # 미탐험 타일 우선
                        if not getattr(tile, 'explored', True):
                            return game_action
                
                # 미탐험 없으면 랜덤 이동
                possible = [(a, dx, dy) for a, dx, dy in directions 
                           if current_dungeon.get_tile(px+dx, py+dy) 
                           and current_dungeon.get_tile(px+dx, py+dy).walkable]
                if possible:
                    return rnd.choice(possible)[0]
            
            # 계단 방향으로 이동 (A* 사용하되 가끔 우회)
            if stairs_pos and (not current_path or rnd.random() < 0.1):
                current_path.clear()
