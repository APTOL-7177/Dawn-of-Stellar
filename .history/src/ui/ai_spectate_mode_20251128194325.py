    ...
    def ai_exploration_input(exploration_sys, party, inv) -> GameAction:
        ...
        try:
            ...
            if not current_path:
                ...
                set_ai_action(f"🎯 목표: {action.reasoning}")
                
                # 목표 위치 설정
                if action.action_type == "move" and hasattr(action, 'target_position'):
                    ai_goal = action.target_position
                elif stairs_pos:
                    ai_goal = stairs_pos  # 기본: 계단
                
                # A* 경로 계산
                if ai_goal:
                    path = astar_pathfind(current_dungeon, (px, py), ai_goal)
                    current_path.extend(path)
                    log_action(f"경로 설정: {ai_goal} ({len(path)}스텝)")
