...
def run_ai_spectate_mode(console: tcod.console.Console, context: tcod.context.Context) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.error(f"[AI] LLM 초기화 실패: {e}")
        add_ai_commentary("❌ LLM 연동 실패!")
        add_ai_commentary("Ollama가 실행 중인지 확인하세요!")
        return
            ...
            if not current_path:
                ...
                try:
                    ...
                    if action.action_type == "move" and action.direction:
                        ...
                        log_action(f"⚠️ LLM 기타 액션: {action.action_type}")
    ...
    def ai_combat_input(ui, combat_manager, current_char, inv) -> GameAction:
        """AI가 전투 입력 결정"""
        import traceback
        log_action(f"🎯 AI 전투 입력 호출: {current_char.name}")
        
        try:
            # LLMPlayerBot 직접 생성하여 사용
            from src.multiplayer.llm_player_bot import create_llm_bot
            bot = create_llm_bot(current_char.name, getattr(current_char, 'job_id', 'warrior'), "qwen3:0.6b", ai.config.play_style)
            ...
            # LLM 전투 결정 - 디버깅 로그 추가
            try:
                log_action(f"⚔️ LLM 전투 호출: {current_char.name}")
                ...
                log_action(f"✅ LLM 전투 결정: {current_char.name} -> {action_name}")
                set_ai_action(f"⚔️ {action_name}")
            except Exception as e:
                log_bug("LLMCombatError", str(e))
                log_action("❌ LLM 전투 실패 - CONFIRM")
                return GameAction.CONFIRM
