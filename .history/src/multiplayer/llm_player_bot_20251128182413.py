...
class LLMConfig:
    ...
    max_tokens: int = 1024  # 최대 출력 토큰 (줄여서 빠르게)
    context_length: int = 2048  # 컨텍스트 길이 (2K로 줄여서 빠르게)
...
SYSTEM_PROMPT_BASE = """게임 AI. BRV 쌓고 HP 공격. BREAK된 적 우선 공격. HP 낮으면 회복.
JSON으로만 응답: {"action":"brv_attack|hp_attack|skill|item|defend","target":"대상","reasoning":"이유"}
"""
...
def create_combat_prompt(state: CombatState, job_info: Dict[str, Any]) -> str:
    ...
    # 현재 캐릭터 상태 (간단히)
    me = f"{current.name}: HP{current.hp}/{current.max_hp} BRV{current.brv}" if current else ""
    ...
    for e in state.enemies:
        if e.is_alive:
            ...
            enemies.append(f"{e.name}:HP{e.hp} BRV{e.brv}{status}")
    
    # 스킬 (ID만)
    skills = [s.id for s in state.available_skills[:3]] if state.available_skills else []
