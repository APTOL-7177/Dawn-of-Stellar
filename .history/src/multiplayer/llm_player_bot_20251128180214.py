...
class LLMConfig:
    ...
    context_length: int = 8192  # 컨텍스트 길이 (8K 권장, 최대 32K)
    retry_count: int = 3  # 재시도 횟수
    enable_thinking: bool = False  # Qwen3 thinking 모드 (비활성화 권장 - 응답 길어짐)
...
def get_system_prompt(style: PlayStyle = PlayStyle.BALANCED, enable_commentary: bool = True) -> str:
    """플레이 스타일에 맞는 시스템 프롬프트 생성"""
    prompt = SYSTEM_PROMPT_BASE
    prompt += STYLE_PROMPTS.get(style, STYLE_PROMPTS[PlayStyle.BALANCED])
    prompt += RESPONSE_FORMAT
    if enable_commentary:
        prompt += "\n\n💬 commentary 필드에 재미있고 유익한 해설을 추가해주세요!"
    return prompt
    ...
    def generate(self, prompt: str, system_prompt: str = "") -> str:
        ...
        payload = {
            ...
            "options": {
                "temperature": self.config.temperature,
                "num_predict": self.config.max_tokens,
                "num_ctx": self.config.context_length,  # 컨텍스트 길이 설정
            }
        }
