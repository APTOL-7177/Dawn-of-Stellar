                            ...
                            if result == "quit":
                                ...
                                if combat_result == CombatState.VICTORY:
                                    ...
                                    if is_boss_fight and (floor_number == 20 or floor_number == 30):
                                        ...
                                        # WorldUI 생성자 인자 수정 (screen_width, screen_height, exploration 추가)
                                        world_ui = WorldUI(
                                            display.console.width, 
                                            display.console.height, 
                                            exploration,
                                            inventory=inventory,
                                        ...
                                        # return_to_town 메서드 존재 여부 확인 후 호출 (없으면 transition_to_town 호출 시도)
                                        if hasattr(world_ui, 'return_to_town'):
                                            world_ui.return_to_town()
                                        elif hasattr(world_ui, 'transition_to_town'):
                                            world_ui.transition_to_town()
                                        else:
                                            # 직접 층 이동 처리
                                            exploration.is_town = True
                                            logger.info("마을로 직접 전환됨")
                            ...
                            if combat_result == CombatState.VICTORY:
                                ...
                                if is_boss_fight and (floor_number == 20 or floor_number == 30):
                                    ...
                                    world_ui = WorldUI(
                                        ...
                                        exploration,
                                        inventory=inventory,
                                        party=character_party if 'character_party' in locals() else (party_members if 'party_members' in locals() else []),
                                        network_manager=network_manager if 'network_manager' in locals() else None,
                                        local_player_id=local_player_id if 'local_player_id' in locals() else None
                                    )
                                    # 메서드 존재 여부 확인 후 호출
                                    if hasattr(world_ui, 'return_to_town'):
                                        world_ui.return_to_town()
